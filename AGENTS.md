...
*   `src/pyschemaelectrical/`: Core library package.
    ...
    *   `primitives.py`: Graphical primitives (`Line`, `Circle`, `Text`, `Polygon`, `Group`, `Path`).
    *   `renderer.py`: SVG rendering engine that converts Elements to SVG XML.
        *   `render_to_svg()`: High-level function to render elements to SVG file
        *   `to_xml_element()`: Converts Elements to XML ElementTree
        *   `calculate_bounds()`: Computes bounding box for auto-sizing
        *   Supports auto-sizing with `width="auto"` and/or `height="auto"`
    *   `autonumbering.py`: State-based component and terminal numbering utilities.
        *   `create_autonumberer()`: Creates initial state
        *   `next_tag()`: Auto-increments component tags (F1, Q1, etc.)
        *   `next_terminal_pins()`: Auto-increments terminal pins per tag
        *   `auto_*_pins()`: Static pin number generators for various components
...
**Terminal Pins**:
- Use `next_terminal_pins(state, terminal_tag, poles)` for auto-incrementing pin numbers per terminal
- Each terminal tag maintains its own independent counter
- Returns a tuple of sequential pin numbers without empty strings
- For 3-pole terminals with tag "X1": first call returns `('1', '2', '3')`, second call returns `('4', '5', '6')`
- For 1-pole terminals with tag "X10": first call returns `('1',)`, second call returns `('2',)`
- Different terminal tags have independent counters (X1, X2, X10 all start from 1)
