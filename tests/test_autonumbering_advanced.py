...
class TestAutonumberingAdvanced:
    ...
    def test_get_pin_counter_unused(self):
        """Test that pin_counter field exists but is not used by current implementation."""
        state = create_autonumberer()
        
        # Initial counter
        assert get_pin_counter(state) == 0
        
        # Counter should not change with terminal_pins operations
        state, _ = next_terminal_pins(state, "X1", poles=3)
        # Note: current implementation uses terminal_counters, not pin_counter
        assert get_pin_counter(state) == 0
