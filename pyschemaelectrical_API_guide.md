...
This guide provides a comprehensive understanding of the `PySchemaElectrical` library. It covers internal mechanics, edge cases, the full API surface, and every layer from low-level symbol factories to high-level circuit descriptors.
...
1. **High-Level API** (`std_circuits/`, `builder.py`, `descriptors.py`): For library consumers. Creates complete circuits with automatic state management, autonumbering, layout, and connections.
...
## 2. Three Ways to Build Circuits

PySchemaElectrical provides three API levels, from simplest to most flexible:

### 2.1. Descriptors (Simplest)
...
### 2.3. Standard Circuits (Pre-Built)
For common industrial circuits. Each is a factory function.
```python
...
### 3.2. Adding Components (`add_component`)
...
### 3.3. Adding Terminals (`add_terminal`)
...
### 3.4. Adding References (`add_reference`)
...
### 3.5. Placing Components Horizontally (`place_right`)
...
## 4. Standard Circuits Library

The functions in `pyschemaelectrical.std_circuits` are pre-built using `CircuitBuilder`. They all share a common signature pattern:

```python
def factory(state, x, y, tm_..., ..., count=1, wire_labels=None, **kwargs) -> Tuple[state, Circuit, used_terminals]
```

### 4.1. `dol_starter` — Direct-On-Line Motor Starter
...
std_circuits.dol_starter(
    ...
    # Layout
    spacing=150.0,             # CircuitLayouts.DOL_STARTER
    symbol_spacing=50.0,
...
### 4.3. `changeover` — Dual-Input Changeover Circuit

```python
std_circuits.changeover(
    state, x, y,
    tm_top_left="X1",          # Input 1 (Main)
    tm_top_right="X2",         # Input 2 (Emergency)
    tm_bot="X3",               # Output
    spacing=150.0,
    symbol_spacing=60.0,
    terminal_offset=20.0,
    tag_prefix="Q",
    count=1,
    wire_labels=None,
)
...
### 4.4. `power_distribution` — Power Distribution

```python
std_circuits.power_distribution(
    state, x, y,
    tm_top="X1",               # Power input terminal
    terminals_list=["X2"],     # List of output terminal IDs
    spacing=80.0,
    symbol_spacing=40.0,
    tag_prefix="F",
    count=1,
...
### 4.5. `spdt` — SPDT Control Circuit (Coil + Changeover Contact)

```python
std_circuits.spdt(
    state, x, y,
    tm_top="X1",               # Input terminal
    tm_bot_left="X2",          # NC output
    tm_bot_right="X3",         # NO output
    spacing=100.0,
    symbol_spacing=50.0,
    column_offset=30.0,
    coil_tag_prefix="Q",
    contact_tag_prefix="K",
    coil_pins=("A1", "A2"),
    contact_pins=("1", "2", "4"),
    count=1,
    wire_labels=None,
    relay_tag=None,            # Fixed relay tag (e.g., "K2")
)
```

### 4.6. `no_contact` — Normally Open Contact Circuit
...
### 4.8. `emergency_stop` — Emergency Stop Circuit
...
### Standard Circuits: Injection Mechanisms

#### The `terminal_maps` Override
Redirect internal logical connections to arbitrary physical terminals.
```python
# Override logical terminal names at build time
std_circuits.psu(..., terminal_maps={"PSU_INPUT": "X99"})
```
...
The `Terminal` dataclass replaces string-based terminal IDs with metadata-rich objects that remain backwards-compatible with strings.
...
**Terminal attributes:**
- `id` (str): Terminal identifier (e.g., "X001")
...
counts = plc.module_count                     # Dict[str, int]
...
## 8. Terminal Output & Exports

### 8.1. Connection Registry (The Netlist)
...
# Expand "1-3" to ["1", "2", "3"]
pins = expand_range_to_pins("1-3", base=1)

# Get connection groups from registry
groups = get_connection_groups_for_terminal("X1", registry)

# Generate internal connections and update CSV
connections = generate_internal_connections_data(...)
update_csv_with_internal_connections("connections.csv", connections)
```

---

## 9. Complete Symbol Reference
...
| `three_pole_spdt_symbol` | 3 | `"{pole}_{type}"` (e.g., `"1_com"`, `"1_nc"`, `"1_no"`) | 9 pins |
...
| Function | Poles | Ports |
|---|---|---|
| `circuit_breaker_symbol` | 1 | `"1"` (up), `"2"` (down) |
| `two_pole_circuit_breaker_symbol` | 2 | `"1"`-`"4"` |
| `three_pole_circuit_breaker_symbol` | 3 | `"1"`-`"6"` |

### Protection (`symbols.protection`)
| Function | Poles | Ports |
|---|---|---|
| `thermal_overload_symbol` | 1 | `"1"` (up), `"2"` (down) |
| `three_pole_thermal_overload_symbol` | 3 | `"1"`-`"6"` |
...
| `three_pole_motor_symbol` | Named per phase + PE | `("U","V","W","PE")` |
...
| `three_pole_terminal_symbol` | 3 | `"1"`-`"6"` (two ports per pole) |
...
|---|---|---|
| `contactor_symbol` | 3-pole NO + coil | Mechanical linkage (stippled line) |
...
- `coil_pins` (tuple): Pins for the coil side
- `contact_pins` (tuple): Pins for the 3-pole contact side

### References (`symbols.references`)
| Function | Notes |
|---|---|
| `ref_symbol` | Arrow for cross-references. `direction`: "up" or "down". Port: "1" (up) or "2" (down) |
...
| Function | Notes |
|---|---|
| `current_transducer_symbol` | Circle with line, no ports |
| `current_transducer_assembly_symbol` | Transducer + terminal box, ports from box |

---

## 10. Model & Constants Reference
...
```python
from pyschemaelectrical import StandardSpacing, StandardTags, StandardPins

# Spacing configs
StandardSpacing.MOTOR          # circuit=150, symbols_start=50, symbols=60
StandardSpacing.SINGLE_POLE    # circuit=100, symbols_start=50, symbols=60
StandardSpacing.POWER_DISTRIBUTION  # circuit=80, symbols_start=50, symbols=40
...
CircuitLayouts.PSU             # spacing=150, symbol=60, terminal_offset=15
CircuitLayouts.CHANGEOVER      # spacing=150, symbol=60, terminal_offset=20
CircuitLayouts.DOL_STARTER     # spacing=150, symbol=50
CircuitLayouts.MOTOR_CONTROL   # spacing=100, symbol=50, column_offset=30
CircuitLayouts.SWITCH          # spacing=100, symbol=60
CircuitLayouts.EMERGENCY_STOP  # spacing=100, symbol=50
```

---

## 11. State Management & Autonumbering
...
```python
state = create_autonumberer()
state, c1, _ = std_circuits.dol_starter(state, tm_top="X1", ...)
state, c2, _ = std_circuits.dol_starter(state, tm_top="X1", ...)
...
## 15. Recipe: Complex Control Loop
